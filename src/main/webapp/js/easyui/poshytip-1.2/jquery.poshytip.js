	...
	$.Poshytip.prototype = {
		...
		show: function() {
			if (this.disabled || this.$tip.data('active'))
				return;
			//alert("111"+this.opts.gridId);
			//判断当前对象是否处于编辑状态如果是则不提示
			if (this.opts.gridId){
				var targetGrid=$("#"+this.opts.gridId);
				//判断是datagrid还是treegrid
				var dataFlag="1";
				try{
					var treeRoot=targetGrid.treegrid("getRoots");
				...
				//datagrid
				if (dataFlag=="1"){
					var rowData=targetGrid.datagrid("getSelected");	
					if (rowData){
						var rowIndex=targetGrid.datagrid("getRowIndex",rowData);
						var editors=targetGrid.datagrid("getEditors",rowIndex);	
				...
				//treegrid
				if (dataFlag=="2"){
					var rowData=targetGrid.treegrid("getSelected");	
					if (rowData){
						var rowId=rowData.id;
						var editors=targetGrid.treegrid("getEditors",rowId);	
						if (editors && editors.length>0){
							return;	
						}
					}
